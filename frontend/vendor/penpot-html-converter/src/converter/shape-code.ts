// Local TypeScript port of @penpot-tools/converter `shape-code.ts` — turns
// the converter output (raw HTML with inline `style="…"` attributes) into
...
//
// This file is maintained IN-TREE, not snapshotted from upstream (see
// VERSION). It deliberately diverges from upstream `shape-code.ts`:
//
//   - No oxfmt — output is unformatted but valid; the Penpot consumers
//     (`export_modal.cljs`, `components.cljs`) run it through the shared
//     `code-beautify` formatter instead.
//   - No `pageToCode` — the Penpot integration only exports single shapes.
//   - `resolveTagOverrides` validates `rule.tag` against `SEMANTIC_TAGS`
//     (upstream trusts its callers; our rules come from user-editable
//     storage, so we re-check here).
//   - `shapeToCode` runs a final `class=` → `className=` sweep for JSX so
//     raw class attrs emitted by shape-specific renderers stay consistent.
//
// When syncing the vendor snapshot (`scripts/sync-html-converter.sh`) this
// file is preserved as-is; diff upstream's shape-code.ts manually and port
// over anything relevant.
...
const SEMANTIC_TAG_SET: ReadonlySet<string> = new Set(SEMANTIC_TAGS);
 ...
 */
export async function shapeToCode(
  shape: Shape,
  allObjects: Record<string, Shape>,
  ctx: ConverterContext,
  options: ShapeCodeOptions,
): Promise<ShapeCodeResult> {
  const innerCtx: ConverterContext = { ...ctx, format: false };
...
export function stylesToCssClasses(
  ...
  function take(decoded: string, preferredSlug: string | null): string {
  ...
  let next = html.replace(
    STYLE_WITH_ID_RE,
    (_m, prefix: string, shapeId: string, declarations: string) => {
  ...
  next = next.replace(
    STYLE_ATTR_RE,
    (_m, declarations: string, offset: number) => {
      const decoded = decodeHtmlEntities(declarations).trim();
      if (!decoded) return '';
...
export function stylesToTailwind(html: string, classAttr: 'class' | 'className'): string {
  return html.replace(STYLE_ATTR_RE, (_m, declarations: string) => {
...
function arb(prop: string, value: string): string { return `[${prop}:${tw(value)}]`; }
 ...
 */
export function declToTailwind(prop: string, v: string): string {
  switch (prop) {
    case 'display':
