...
function normalize(font) {
    ...
    if (!fontFamily)
        return null;
    if (GENERIC_FAMILIES.has(fontFamily.toLowerCase()))
        return null;
...
function isGoogleFont(fontId) {
    return fontId.startsWith('gfont-');
}
const SOURCE_SANS_PRO_SUFFIXES = {
    200: 'extralight',
    300: 'light',
    400: 'regular',
    700: 'bold',
...
function sourceSansProSuffix(weight, italic) {
...
function bundledFontFace(font, baseUrl) {
...
function gfontVariantId(weight, italic) {
    if (weight === 400)
        return italic ? 'italic' : 'regular';
    return italic ? `${weight}italic` : String(weight);
}
async function fetchGfontsCss(family, variants, baseUrl, fetchFn) {
 ...
 */
export async function buildPenpotFontsCss(fonts, options = {}) {
    const baseUrl = (options.baseUrl ?? DEFAULT_BASE_URL).replace(/\/$/, '');
    const fetchFn = options.fetch ?? globalThis.fetch;
    const cssParts = [];
    const gfontsByFamily = new Map();
    for (const raw of fonts) {
        ...
        if (isGoogleFont(font.fontId)) {
            ...
            const set = gfontsByFamily.get(font.fontFamily) ?? new Set();
            set.add(variant);
