...
export function resolveTagOverrides(rules, objects) {
    ...
    for (const id in objects) {
        ...
        const containsMatch = byContains.find((r) => name.includes(r.value.toLowerCase()));
        if (containsMatch)
            out.set(shape.id, containsMatch.tag);
    }
    return out;
}
/**
...
export function declToTailwind(prop, v) {
    switch (prop) {
        ...
        case 'flex':
            if (v === '1' || v === '1 1 0%')
                return 'flex-1';
            if (v === 'auto' || v === '1 1 auto')
                return 'flex-auto';
            if (v === 'none' || v === '0 0 auto')
                return 'flex-none';
            return arb(prop, v);
        case 'flex-shrink':
            if (v === '0')
...
//# sourceMappingURL=shape-code.js.map