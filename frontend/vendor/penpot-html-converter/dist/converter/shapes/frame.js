...
import { layoutItemSizingStyle, layoutItemMarginStyle, layoutItemAlignSelfStyle, layoutItemMinMaxStyle, layoutItemZIndexStyle, layoutItemAbsoluteStyle, } from '../layout/layout-item';
...
export function invTransformForChildren(shape) {
    if (!shape.transform || isIdentityMatrix(shape.transform))
        return undefined;
    return invertMatrix(shapePageTransform(shape));
}
function shadowBorderRadiusFromChildren(shape, children) {
    if (!shape.shadow?.length)
        return '';
    const hasOwnRadius = (shape.r1 ?? 0) !== 0 ||
        (shape.r2 ?? 0) !== 0 ||
        (shape.r3 ?? 0) !== 0 ||
        (shape.r4 ?? 0) !== 0;
    if (hasOwnRadius)
    ...
    for (const child of children) {
        ...
        if (child.type !== 'rect' && child.type !== 'circle')
            continue;
        if ((child.width ?? 0) !== shape.width || (child.height ?? 0) !== shape.height)
            continue;
        const r = child.r1 ?? 0;
        if (r === 0)
            continue;
        if ((child.r2 ?? 0) !== r || (child.r3 ?? 0) !== r || (child.r4 ?? 0) !== r)
            continue;
        return decl.borderRadius(r);
    }
    return '';
}
export function renderFrame(shape, children, objects, ctx) {
    const isRoot = shape.parentId === shape.id;
    const rawLayout = shape.layout;
    ...
    const clipStyle = shape.clipContent !== false && !shape.showContent ? decl.overflow('hidden') : '';
    ...
    let positionStyle = '';
    if (isRoot) {
        positionStyle = mergeStyles(decl.position('relative'), decl.width(shape.width), decl.height(shape.height));
    }
    else {
    ...
    const hasAbsoluteChild = (isFlex || isGrid) &&
        children.some((c) => c.layoutItemAbsolute);
    ...
    const plainFrameNeedsRelative = !isFlex && !isGrid && !isRoot && children.length > 0 && ctx._parentIsLayout;
    ...
    const needsContainingBlock = !shape.layoutItemAbsolute &&
    ...
    // `isolation: isolate` forces the frame to create its own stacking
    // context. Without it, a child with `z-index: -1` would slip past
    // the frame and paint behind whatever ancestor *does* establish a
    // stacking context — the classic "z-index: -1 disappears" gotcha.
    // With it, every Penpot frame matches what designers expect from
    // the workspace: child stacking stays scoped to that frame.
    const isolateStyle = 'isolation: isolate;';
    const style = mergeStyles(positionStyle, extraPositionStyle, layoutStyle, base, shadowRadius, fills, stroke, clipStyle, bgStyle, isolateStyle);
    let inner;
    if (isFlex) {
        ...
        inner = orderedChildren
            ...
            const itemStyle = mergeStyles(minResetStyle, sizingStyle, layoutItemMarginStyle(child), layoutItemAlignSelfStyle(child), layoutItemMinMaxStyle(child), layoutItemZIndexStyle(child), layoutItemAbsoluteStyle(child, frameOffsetX, frameOffsetY));
            const flexCtx = {
                ...ctx,
    ...
    return tag(ctx.tagOverride?.(shape) ?? 'div', { 'data-id': shape.id, 'data-type': shape.type, style: style || undefined }, inner);
}
