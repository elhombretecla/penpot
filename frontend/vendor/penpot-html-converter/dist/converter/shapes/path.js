...
function buildMarkerDef(capType, color, id) {
    if (!capType || capType === 'round' || capType === 'square')
        return null;
    let inner;
    let viewBox;
    let refX;
    ...
    return tag('marker', { id, viewBox, refX, refY, markerWidth, markerHeight, orient: 'auto-start-reverse' }, inner);
}
export function renderPath(shape, ctx) {
    ...
    const pathStyleParts = [];
    if (linecap)
        pathStyleParts.push(`stroke-linecap:${linecap}`);
    if (markerStartAttr)
        pathStyleParts.push(`marker-start:${markerStartAttr}`);
    if (markerEndAttr)
        pathStyleParts.push(`marker-end:${markerEndAttr}`);
