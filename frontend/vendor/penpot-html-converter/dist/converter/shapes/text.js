...
export function textLeafToStyles(leaf, typographies) {
    const resolved = {};
    if (typographies && leaf.typographyRefId) {
        const typo = typographies[leaf.typographyRefId];
        if (typo)
            Object.assign(resolved, typo);
    }
    Object.assign(resolved, leaf);
    const parts = [];
    if (resolved.fontSize)
        parts.push(decl.fontSize(Number(resolved.fontSize)));
    if (resolved.fontWeight)
        parts.push(decl.fontWeight(resolved.fontWeight));
    ...
    if (resolved.lineHeight)
        parts.push(decl.lineHeight(resolved.lineHeight));
    if (resolved.textAlign) {
        parts.push(decl.textAlign(resolved.textAlign));
    }
    if (resolved.fontStyle === 'italic')
        parts.push(decl.fontStyle('italic'));
    if (resolved.textDecoration === 'underline')
        parts.push(decl.textDecoration('underline'));
    if (resolved.textDecoration === 'line-through')
        parts.push(decl.textDecoration('line-through'));
    if (resolved.textTransform) {
        parts.push(decl.textTransform(resolved.textTransform));
    }
...
export function textLeafColorStyle(leaf, fillTokenName, tokens, fallbackColor) {
    if (fillTokenName)
        return decl.color(tokenToCssVar(fillTokenName, tokens));
...
export function renderParagraph(para, fillTokenName, tokens, fallbackColor, typographies) {
    const paraLeaf = { text: '', ...para };
...
function collectLeafFont(leaf, collector, typographies) {
    const resolved = {};
    if (typographies && leaf.typographyRefId) {
        const typo = typographies[leaf.typographyRefId];
        if (typo)
...
function collectTextFonts(shape, ctx) {
    if (!ctx._fontCollector || !shape.content)
        return;
...
function firstNonEmptyLeaf(shape) {
    if (!shape.content)
        return null;
    for (const set of shape.content.children) {
        for (const para of set.children) {
            for (const leaf of para.children) {
                if (leaf.text !== undefined && leaf.text !== '')
                    return { leaf, para };
            }
        }
    }
...
export function renderText(shape, ctx) {
    ...
    if (shape.width !== undefined)
        sizeParts.push(decl.width(shape.width));
    if (shape.height !== undefined)
        sizeParts.push(decl.height(shape.height));
    ...
    const verticalAlignStyle = verticalAlign === 'center'
