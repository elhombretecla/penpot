...
export function gridTracksToStyle(tracks, axis) {
    if (tracks.length === 0)
        return '';
...
export function gridCellStyle(cell) {
    const parts = [];
    parts.push(decl.gridRowStart(cell.row));
    if (cell.rowSpan > 1)
        parts.push(decl.gridRowEnd(`span ${cell.rowSpan}`));
    parts.push(decl.gridColumnStart(cell.column));
    if (cell.columnSpan > 1)
        parts.push(decl.gridColumnEnd(`span ${cell.columnSpan}`));
...
export function findCellForShape(frame, shapeId) {
    if (!frame.layoutGridCells)
        return undefined;
