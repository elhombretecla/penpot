...
## 7. Experiment: the compact binfile-v3 format (branch `file-format`, July 2026)

Implementation of levers §6.A1–A3 + §6.B5, measured on real files. All changes live in
the backend (+`common/`); `@penpot/library` and the frontend are untouched. The importer
reads **both** layouts forever (detection is structural, not version-based).

### What changed

| Change | Where |
|---|---|
| Export no longer pretty-prints JSON (`:indent true` removed) | `backend/src/app/binfile/v3.clj` `write-entry!` |
| Import indexes all ZIP entries in ONE pass (was: full regex scan per section *and per page* — O(sections × entries)) | `v3.clj` `read-entry-index` / `index-entry` |
| **Compact layout**: one JSON entry per page, `objects` embedded (`files/{fileId}/pages/{pageId}.json`), no per-shape entries, no `pageId` per shape; manifest `version: 2` (hint only — importer detects `objects` in the page JSON) | `v3.clj` export/import, flag `binfile-v3-compact-export` |
| **Shape pruning** on export + reconstruction on import: identity `transform`/`transformInverse` omitted; `selrect`+`points` omitted when derivable (identity transform, no rotation; for path/bool: `selrect` kept, `points`+`x/y/w/h` dropped); nils dropped. Restored via `expand-shape` (uses `setup-rect`/`setup-path`) in `clean-shape-post-decode`, i.e. *before* any validation | `common/src/app/common/files/shape_compact.cljc`, `backend/src/app/binfile/cleaner.clj` |
| **Float rounding** to 4 decimals on the encoded JSON (kills `0.6000000238418579` float32 artifacts) | same ns, sub-flag `binfile-v3-round-floats` |

Enable with `PENPOT_FLAGS="... enable-binfile-v3-compact-export enable-binfile-v3-round-floats"`.

### Measured results (devenv, this branch)

Codex (105 567 shapes, 52 pages, 3 339 components):

| | original (library) | legacy re-export (no indent) | **compact** |
|---|---:|---:|---:|
| Size | 123.9 MiB | 112.8 MiB | **33.8 MiB (−73 %)** |
| ZIP entries | 109 075 | 109 075 | **3 508** |
| ZIP structural overhead | 44.7 MiB (36 %) | 35.8 MiB (32 %) | **0.9 MiB (2.8 %)** |
| Export time | — | 15.3 s | **11.2 s** |
| Import time | 40.6 s | — | **12.8 s** |

CYGNUS (215 822 shapes, 70 pages, 8 361 components):

| | original (library) | **compact** |
|---|---:|---:|
| Size | 280.6 MiB | **96.6 MiB (−66 %)** |
| ZIP entries | 224 625 | **8 803** |
| ZIP structural overhead | 92.0 MiB (33 %) | **2.3 MiB (2.4 %)** |
| Import time | 56.9 s | **32.4 s** |
| Export time (compact) | — | 24.7 s |

Round-trip integrity (Codex, original import vs compact export→import): pages, page
order, shape count, all shape/component ids **identical**; per-attribute diff over all
105 567 shapes classified every difference as (a) numeric noise < 0.001 from the opt-in
rounding, or (b) `componentFile`/`fills` ref-file uuids — expected, every import remaps
file ids. `ctf/check-file` (referential integrity) passes on every import.

**Render validation (headless Chromium via Playwright against the devenv workspace,
render-wasm):** CYGNUS original import vs compact round-trip, same page, same viewport —
cover page: 193 of 2 073 600 pixels differ (0.009 %, sub-pixel antialiasing from the
opt-in rounding); "Modals" page (8 475 shapes, dense component instances):
**pixel-identical (0 differing pixels)**. The ~50k-shape "External components" page
cannot be screenshot in headless (no GPU) for *either* file — symmetric environment
limitation, not a format issue. Scripts: `sample-files/render-check*.mjs`.

**Pre-existing quirk found (NOT introduced here, verified on legacy round-trip too):**
`svgAttrs` keys are camelCase in the DB (`:fillRule`), the JSON reader kebab-cases every
key on import (`:fill-rule`), and the migration that re-camelizes them
(`0021-fix-shape-svg-attrs`) only runs once per file — so ANY binfile-v3 round-trip
kebab-cases `svgAttrs` keys. Candidate upstream fix: re-apply `csvg/attrs->props` in the
import cleaner.

### Compatibility

Old Penpot versions cannot import compact files (loud validation failure, no silent
corruption). This branch imports both layouts. `@penpot/library` still emits the legacy
layout (updating it = future work; `compact-shape`/`expand-shape` are in `common/` so it
can reuse them).

### Next step (future work): compact format in `@penpot/library` → converter

The biggest remaining win is teaching `@penpot/library` (source: `library/` in this
repo) to *write* the compact layout, so `.fig`→`.penpot` converter outputs shrink ~3×
with zero converter changes (it writes through `exportStream`). CYGNUS-class design
systems would drop under the 120 MiB import cap without `--split`.

Concrete pointers, in implementation order:

1. `library/src/lib/export.cljs` `generate-file-export-procs` (~:96-156) is the whole
   layout: today it emits the page JSON *without* `:objects` plus one proc per shape via
   `encode-shape*` (~:88-94, which also converts path/bool `:content` to vectors —
   keep that). Compact mode = one proc per page emitting
   `(assoc page :index i :objects <compacted+encoded shapes>)` and NO per-shape procs;
   drop the per-shape `:page-id`.
2. Reuse `app.common.files.shape-compact/compact-shape` + `round-values` — they are
   `.cljc` in `common/` precisely so the library shares the exact same pruning rules as
   the backend exporter (and the backend's `expand-shape` restores them on import).
   Apply `round-values` after `encode-shape*`, before `json/encode`.
3. Manifest: `:version 2` in `generate-manifest-procs` (~:184-202) when compact.
4. API: a `:format "compact" | "legacy"` option on `exportStream`/`exportBytes`/
   `exportBlob`. **Default must stay `legacy`** until the instances that will import
   these files (SaaS included) ship the structural-detection importer from this branch —
   an old Penpot cannot read compact files. Flip the default in a later release.
5. Tests: `library/test/` export assertions (page entry carries `objects`, no per-shape
   entries, manifest v2) plus a backend round-trip against `bf.v3/import-files!` with a
   library-generated compact ZIP (see `backend_tests.binfile-test/export-binfile-v3-compact`
   for the shape of that test).
6. Remember `mem:library/core`: the library has its own compatibility defaults for
   features/migrations — do not assume it exports with the app's newest defaults.

### Tooling / tests

- Census + timing: `backend/dev/binfile_bench.clj` — standalone census:
  `clojure -Sdeps '{:deps {org.clojure/clojure {:mvn/version "1.12.0"}} :paths []}' -M -e "(load-file \"dev/binfile_bench.clj\")(binfile-bench/print-census \"file.penpot\")"`
- `common/test/common_tests/files_shape_compact_test.cljc` (JVM+JS, generative over
  `cts/schema:shape`) and `backend/test/backend_tests/binfile_test.clj`
  (`export-binfile-v3-compact` round-trip; legacy round-trip still green).
